...
def upload_chunk_namelists(run_uuid):
    """Phase 3 unified mode: upload ONLY namelist.input + namelist.wps to inputs/<run_uuid>/.

    The wrf*input/bdy/fdda/lowinp/trmask files are local-only in unified mode (regenerated
    each chunk), so they're not uploaded. Replaces upload_run_inputs() for the chunked flow.
    """
    resolved = _resolve_remote_output()
    if resolved is None:
        return
