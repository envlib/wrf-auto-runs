...
# =============================================================================
# WRF-Auto Pipeline — Apptainer on Hetzner (Job Array, CSV-based, Fixed Duration)
#
# Submits multiple WRF runs as a Slurm job array. Each task reads its
# start_date from a CSV file (start_dates.csv) using SLURM_ARRAY_TASK_ID as
# a 0-based row index. All tasks share a single DURATION_HOURS value set
# below.
#
# CSV format (slurm_scripts/start_dates.csv):
#   start_date
#   2020-01-01 00:00:00
#   2020-01-03 00:00:00
#   ...
#
# Usage:
#   sbatch slurm_scripts/run_wrf_hetzner_csv_array_duration.sl
#   sbatch --array=0-5 slurm_scripts/run_wrf_hetzner_csv_array_duration.sl
#   CSV_FILE=/path/to/dates.csv sbatch slurm_scripts/run_wrf_hetzner_csv_array_duration.sl
#
# To auto-detect array range from CSV row count:
#   ROWS=$(( $(wc -l < slurm_scripts/start_dates.csv) - 1 ))
#   sbatch --array=0-$((ROWS - 1)) slurm_scripts/run_wrf_hetzner_csv_array_duration.sl
#
...
CSV_FILE="${CSV_FILE:-$(pwd)/start_dates.csv}"

# Global simulation duration (hours) shared by every task in the array.
DURATION_HOURS=48
...
if [ -z "${START_DATE}" ]; then
    echo "ERROR: Failed to parse start_date from CSV line $((SLURM_ARRAY_TASK_ID + 2)): '${CSV_LINE}'"
    exit 1
fi

echo "Array task ${SLURM_ARRAY_TASK_ID}: start_date=${START_DATE}, duration_hours=${DURATION_HOURS}"
