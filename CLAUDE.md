...
- **`upload_end_frame`** (default `false`) — the post-run upload skips the single-frame wrfout of a run ending exactly at midnight (`utils.end_frame_min_files`); the hindcast pattern relies on that skip (next week's job rewrites the file and may run first). A single-stage forecast whose last lead is that frame sets it `true`.
- **`[input] kind = 'intermediate', prefix = 'IFS'`** — pre-staged WPS intermediate files `<prefix>:<date>` already in `data_path`: `main.py` skips download/convert, `set_params` sets `fg_name` to the prefix, `run_metgrid` deletes the files after success. `params._resolve_input_kind` refuses `[restart]` (the chunked twin's cleanup would delete the staged files after chunk 1), `[remote.wrf]`, and a non-default `[sst].source` (`[remote.era5]` is ignored); the staging tool owns the extent check. `monitor_wrf.post_run_files` is the post-run selection (testable wiring of `upload_end_frame`). Tests: `tests/test_intermediate_input.py`.
- **`[hooks] on_output_file = '<cmd> {path}'`** — `utils.run_output_hook`, called from `utils.ul_output_files` per file after rclone exit 0 and before `os.remove`, so the hook sees the file under its archived colon-free name. `match` (fnmatch on the basename), `timeout_seconds`, `grace_seconds`; `{path}` is shell-quoted; the hook runs in its own session and on timeout gets SIGTERM → grace → SIGKILL to the process group (never a bare `subprocess.run(timeout=)`, which SIGKILLs and leaves a child's remote lock ticket behind). Never raises (every exception, incl. a bad command string or undecodable output, is caught); failures print and `sentry_sdk.capture_message(level='warning')`; stderr's tail is printed on success too. `monitor_wrf.deliver_output_files` is the single delivery seam: with `[remote.output]` → `ul_output_files` (upload, hook, delete); without it but with a hook → `utils.hook_output_files` (hook, delete on success, keep on failure — a failed file is skipped on later polls and retried once in the post-run pass (`final=True`); non-matching files kept); neither → files stay. Tests: `tests/test_output_hook.py` (real subprocesses for the signal path).
