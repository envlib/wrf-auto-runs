...
| `mullenkamp/wrf-auto-runs-intel-wvt-avx512:1.0` | Intel oneAPI | dmpar | **Multi-region WVT, AVX-512 build** — same pipeline and same WRF source as `intel-wvt`, WRF compiled with `-march=skylake-avx512` (−4% at 0 regions, −6% at 12 on the harness; output byte-identical to `intel-wvt` there). Needs an AVX-512 host (all current Slurm clusters are Zen 4; not Milan / older Intel). Base `wrf-wps-intel-wvt-ubuntu-avx512:1.0`. Build context `intel_wvt_avx512/`. Not pushed. |
