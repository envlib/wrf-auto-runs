...
|---|---|---|---|
| `mullenkamp/wrf-auto-runs-intel-wvt:2.11` | Intel oneAPI | dmpar | **Current — multi-region WVT** (2.11, 2026-09-19: pipeline-only — `[input].kind='intermediate'`, `[hooks].on_output_file`, `upload_end_frame`; same base 2.6.) (1..12 tagged regions incl. 4 lateral-boundary faces via `[[wvt.regions]]`; `num_wvt_regions=1` reproduces single-region bit-for-bit). **2.10 / base 2.6 (2026-09-19; 2.9 / 2.5 the day before, whose source/sink rule wrongly refused tracer-off namelists carrying `tracer2dsource=1`): every WVT state field is Registry-packaged — a `tracer_opt=0` run allocates and writes none of it; `tracer_opt=4` must be on every domain or none; `tracer3dsource`/`tracer3dsink` need `tracer_opt=4`; the pipeline prunes WVT names from `ncks -v` when tracers are off. Base compiled `-march=core-avx2` (any CPU since 2013).** Earlier: 2.5/2.2 added the nine New Tiedtke water-vapour-tag mirrors (output from images before that carries a defective convective tagging, see `wrf-model-eval/docs/wvt_cumulus_tagging.md`); 2.1+ added the face tags. Not pushed. |
