...
- **`upload_end_frame`** — Default `false`. A run that ends exactly at midnight leaves a single-frame wrfout named like a new day; the hindcast pattern skips it (the next week's job rewrites it, and may run first). A single-stage forecast whose last lead lives in that frame sets it `true`. Env var override.
- **`[input]`** — `kind = 'era5'` (default) | `'wrf'` (implied by `[remote.wrf]`) | `'intermediate'`. With `'intermediate'` and `prefix = 'IFS'`, the WPS intermediate files `IFS:<date>` are already in the data directory (staged by an external tool such as `cfdb-to-int`): no download, no conversion, `fg_name` points at the prefix, and the files are removed after metgrid when `cleanup_inputs` is true. Single-stage only; `[sst].source` must be absent (or `'era5'`, the default, meaning "from the input"); a `[remote.era5]` block is ignored; the staging tool owns the extent check.
- **`[hooks]`** — `on_output_file = '<command> {path}'` runs a command per completed output file (`{path}` = the local file under its archived colon-free name, ncks-filtered only when `output_variables` is set); with `[remote.output]` after the upload succeeded and before the local delete. Optional `match` (fnmatch on the basename, default `*`), `timeout_seconds` (900), `grace_seconds` (60). Never raises: a failure, timeout, bad command string or undecodable output is printed (Sentry warning when configured) and WRF continues; the hook runs in its own process group, and a timed-out hook gets SIGTERM, the grace period, then SIGKILL to the whole group. The tail of the hook's stderr is printed on success too, so the pipeline log records what each hook did. Without `[remote.output]` the hook is the *only* consumer: each matching file is filtered/renamed as usual, handed to the hook, and deleted after the hook succeeds (`delete_on_success`, default `true`) or kept when it fails; non-matching files and everything else stay in `run_path` as before.
...
8. Download ERA5 or WRF data via rclone (skipped for `[input].kind = 'intermediate'`)
9. Convert to WPS intermediate format (`era5_to_int` or `wrf_to_int`; skipped when the files are pre-staged)
...
16. Otherwise run `wrf.exe`, poll for completed output files and deliver each: upload to `[remote.output]` (then the `[hooks].on_output_file` command, then delete), or, without a remote, the hook alone (delete on success, keep on failure with one post-run retry), or leave them in `run_path`. The post-run pass skips the midnight single-frame end file unless `upload_end_frame = true`.
...
|---|---|---|---|
| `mullenkamp/wrf-auto-runs-intel-wvt:2.12` | Intel oneAPI | dmpar | **Current — multi-region WVT** (**2.12, 2026-09-24: nested domains update SST** — `auxinput4_interval` is written per domain; every earlier image left d02+ at their initial SST for the whole run, see CLAUDE.md. 2.11, 2026-09-19: pipeline-only — `[input].kind='intermediate'`, `[hooks].on_output_file`, `upload_end_frame`; same base 2.6.) (1..12 tagged regions incl. 4 lateral-boundary faces via `[[wvt.regions]]`; `num_wvt_regions=1` reproduces single-region bit-for-bit). **2.10 / base 2.6 (2026-09-19; 2.9 / 2.5 the day before, whose source/sink rule wrongly refused tracer-off namelists carrying `tracer2dsource=1`): every WVT state field is Registry-packaged — a `tracer_opt=0` run allocates and writes none of it; `tracer_opt=4` must be on every domain or none; `tracer3dsource`/`tracer3dsink` need `tracer_opt=4`; the pipeline prunes WVT names from `ncks -v` when tracers are off. Base compiled `-march=core-avx2` (any CPU since 2013).** Earlier: 2.5/2.2 added the nine New Tiedtke water-vapour-tag mirrors (output from images before that carries a defective convective tagging, see `wrf-model-eval/docs/wvt_cumulus_tagging.md`); 2.1+ added the face tags. Not pushed. |
